    ...
    public class ListAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            if (position == 0) {
                ...
                if (evalInfo.getType() == 1) { // person eval
                ...
                txtDate.setText(CommonUtils.getDateStrFromStrFormat(evalInfo.getWriteTimeString(), "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm"));
                ...
                if (evalInfo.getOwner() == SessionInstance.getInstance().getLoginData().getUser().getId()) {
                    txtError.setVisibility(View.GONE);
                } else {
                    txtError.setVisibility(View.VISIBLE);
                    txtError.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            if (SessionInstance.getInstance().getLoginData().getUser().getTestStatus() == Constants.TEST_STATUS_PASSED) {
                                ...
                                intent.putExtra("estimateId", evalInfo.getId());
                                intent.putExtra("pname", evalInfo.getTargetAccountAkind() == Constants.ACCOUNT_TYPE_PERSON ? evalInfo.getTargetAccountRealname() : evalInfo.getTargetAccountEnterName());
                                intent.putExtra("nname", evalInfo.getOwnerAkind() == Constants.ACCOUNT_TYPE_PERSON ? evalInfo.getOwnerRealname() : evalInfo.getOwnerEnterName());
                                intent.putExtra("ncontent", evalInfo.getContent());
                ...
                txtDate.setText(CommonUtils.getDateStrFromStrFormat(item.getWriteTimeString(), "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm"));
            }
