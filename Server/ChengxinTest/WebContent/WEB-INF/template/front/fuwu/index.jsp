...
<%@ include file="/WEB-INF/template/front/layout/body_top.jsp"%>
            ...
            <div class="col-xs-12 init-div-horizontal" style="margin-top: 48px; padding-bottom:60px;">
                <div class="row init-div blue-gradient" style="padding: 10px 10px 10px 15px;">
                    <div>
                        ...
                        <div style="float:left; margin-left: 10px;">
                            <div class="row init-div">
                                <span id="txt_name" class="text-white font-lg">北京阿里</span>
                ...
                <div class="row init-div">
                    ...
                    <div class="col-xs-12  init-div-horizontal margin-bottom-10 margin-top-10">
                        <div class="col-xs-6 init-div-horizontal">
                            ...
                            <span id="txt_name" class="text-gray font-md">服务介绍</span>
                        </div>
                    ...
                    <div class="col-xs-12">
                        <div class="row init-div margin-bottom-10 margin-top-10">
                            <span class="text-gray-light font-sm">地址：</span><span id="txt_resource" class="text-dark font-md">北京市朝阳区新源里109</span>
                        </div>
                    </div>
...
//            $('#bottombar').css('top', $(window).outerHeight(true) - $('#bottombar').height() - 7);
        ...
        function getServeDetail() {
        	var serviceId = getParameterByName("serviceId");
            $.ajax({
                type: "POST",
                url: "api.html?share=1&pAct=getServiceDetail",
                data: {
                	serviceId: serviceId,
                },
                success: function(res) {
                    ...
                    if (res.retCode == 200) {
                    	...
                    	var img_logo = (service.logo) ? uploadPath + service.logo : "assets/custom/img/no_image.png";
                    	var img_enter_logo = (service.accountLogo) ? uploadPath + service.accountLogo : "assets/custom/img/no_image.png";
                    	$('#img_logo').attr('src' , img_logo);
                    	$('#img_avatar').attr('src' , img_enter_logo);
                    	$('#txt_name').html(service.name);
                    	$('#txt_leixing').html(service.fenleiName);
                    	$('#txt_city').html(service.cityName);
                    	$('#txt_code').html(service.code);
                    	$('#txt_resource').html(service.addr);
                    	$('#txt_comment').html(service.comment);
                    	$('#txt_weburl').html(service.weburl);
                    	$('#txt_addr').html(service.addr);
                    	$('#txt_relation_name').html(service.contactName);
                    	$('#txt_relation_mobile').html(service.contactMobile);
                    	$('#txt_relation_weixin').html(service.contactWeixin);
                    	if(service.akind == 1){
                    		$('#txt_enter_type').html("个人");	
                    	}else{
                    		if(service.enterKind == 2){
                    			$('#txt_enter_type').html("个体户");
                    		}else{
                    			$('#txt_enter_type').html("企业");
                    		}
                    	}
                    	$('#txt_enter_name').html(service.accountName);
                    	$('#txt_enter_code').html(service.accountCode);
                    	$('#txt_write_time').html(service.writeTimeString);
                    	if(service.accountCredit < 0){
                    		$('#txt_enter_rate').html("暂无");
                    	}else{
                    		$('#txt_enter_rate').html(service.accountCredit + "%");
                    	}
