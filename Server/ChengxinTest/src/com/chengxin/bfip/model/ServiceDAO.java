 ...
 */
public class ServiceDAO extends BaseDataAccessObject {

	private static String VIEW = "services_v";


	public void insert(Service member) {
		DBModelUtil.processSecure(Service.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().save(member);
	}

	public void update(Service member) {
		DBModelUtil.processSecure(Service.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().update(member);
	}

	public void delete(Service member) {
		DBModelUtil.processSecure(Service.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
	...
	public Service get(int id) {
		...
		stringBuffer.append("FROM Service ");
		...
		if (list.size() > 0) {
			Service member = (Service)list.get(0);

			DBModelUtil.processSecure(Service.class.getName(), member, DBModelUtil.C_SECURE_TYPE_DECRYPT);
	...
	public Service getDetail(int id) {
		...
		List<Service> resultList = this.search(filterParamObject);
	...
	private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere, String extraOrder, String groupby) {
		...
		if(isCountSQL) {
			...
			sql.appendSQL("SELECT id, code, account_id, name, fenlei_id, fenlei_name, city_id, city_name, province_id, province_name"
            		...
            		+ ", akind, akind_name, enter_kind, enter_kind_name, realname, enter_name"
            		+ ", account_mobile, addr, account_view_cnt, account_credit, account_code, account_name, account_logo ");
		}
	...
	public List<Service> search(JSONObject filterParamObject) {
		return search(filterParamObject, "");
	}
	
	public List<Service> search(JSONObject filterParamObject, String extraWhere) {
    	return search(filterParamObject, extraWhere, "");
    }
    
    public List<Service> search(JSONObject filterParamObject, String extraWhere, String extraOrder) {
    	return search(filterParamObject, extraWhere, extraOrder, "");
    }

	public List<Service> search(JSONObject filterParamObject, String extraWhere, String extraOrder, String groupby) {
		...
		if (queryList != null) {
			...
			for(int i = 0; i < listSize; i++) {
                ...
                row.setWriteTimeString(DateTimeUtil.dateFormat(DateTimeUtil.stringToDate(CommonUtil.toStringDefault(objectArray[26]))));
                ...
                row.setAccountName(CommonUtil.toStringDefault(objectArray[38]));
                row.setAccountLogo(CommonUtil.toStringDefault(objectArray[39]));
                row.setImgPaths(CommonUtil.getImgPathList(objectArray, 21, 25));

				DBModelUtil.processSecure(Service.class.getName(), row, DBModelUtil.C_SECURE_TYPE_DECRYPT);
