 ...
 */
public class VerifyCodeDAO extends BaseDataAccessObject {

    private static String VIEW = "verify_codes_v";
    
    
    public void insert(VerifyCode verifyCode) {
        DBModelUtil.processSecure(User.class.getName(), verifyCode, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
        
        this.getHibernateTemplate().save(verifyCode);
    }

    public void update(VerifyCode verifyCode) {
        DBModelUtil.processSecure(User.class.getName(), verifyCode, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
        
        this.getHibernateTemplate().update(verifyCode);
    }
    
    public void delete(VerifyCode verifyCode) {
        DBModelUtil.processSecure(User.class.getName(), verifyCode, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
        
        this.getHibernateTemplate().delete(verifyCode);
    }
    ...
    public VerifyCode get(int id) {
        StringBuffer stringBuffer = new StringBuffer();

        stringBuffer.append("FROM VerifyCode ");
        ...
        if (list.size() > 0) {
        	VerifyCode verifyCode = (VerifyCode)list.get(0);
            
            DBModelUtil.processSecure(User.class.getName(), verifyCode, DBModelUtil.C_SECURE_TYPE_DECRYPT);
    ...
    public VerifyCode getDetail(int id) {
        ...
        List<VerifyCode> resultList = this.search(filterParamObject);
    ...
    public VerifyCode getDetail(String where) {
        
        List<VerifyCode> resultList = this.search(null, where);
    ...
    private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere, String extraOrder) {
        ...
        if(isCountSQL) {
            ...
            sql.appendSQL("SELECT id, mobile, verify_code, write_time ");
        }
    ...
    public List<VerifyCode> search(JSONObject filterParamObject) {
    	return search(filterParamObject, "");
    }
    
    public List<VerifyCode> search(JSONObject filterParamObject, String extraWhere) {
    	return search(filterParamObject, extraWhere, "");
    }
    
    public List<VerifyCode> search(JSONObject filterParamObject, String extraWhere, String extraOrder) {
        ...
        List<VerifyCode> resultList = new ArrayList<VerifyCode>();
