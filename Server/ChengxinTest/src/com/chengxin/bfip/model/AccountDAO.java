    ...
    private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere, String extraOrder, String groupby) {
        ...
        if(isCountSQL) {
            ...
            sql.appendSQL("SELECT id, mobile, password, akind, code, view_cnt, elect_cnt, logo, enter_kind, enter_name"
            		...
            		+ ", req_code_sender_akind, inviter_friend_level, prov_city, enter_id, friend1_name, xy_watch_name, xy_watched_name"
            		+ ", req_code_sender_mobile, feedback_cnt, req_code_sender_name, invite_cnt ");
        }
    ...
    public List<Account> search(JSONObject filterParamObject, String extraWhere, String extraOrder, String groupby) {
        ...
        if (queryList != null) {
            ...
            for(int i = 0; i < listSize; i++) {
                ...
                row.setEnterId(CommonUtil.toIntDefault(objectArray[54]));
                row.setFriend1Name(CommonUtil.toStringDefault(objectArray[55]));
                row.setXyWatchName(CommonUtil.toStringDefault(objectArray[56]));
                row.setXyWatchedName(CommonUtil.toStringDefault(objectArray[57]));
                row.setReqCodeSenderMobile(CommonUtil.toStringDefault(objectArray[58]));
                row.setFeedbackCnt(CommonUtil.toIntDefault(objectArray[59]));
                row.setReqCodeSenderName(CommonUtil.toStringDefault(objectArray[60]));
                row.setInviteCnt(CommonUtil.toIntDefault(objectArray[61]));
