 ...
 */
public class ReadStatusDAO extends BaseDataAccessObject {

	private static String VIEW = "read_status_v";

	
	public void insert(ReadStatus readStatus) {
		DBModelUtil.processSecure(ReadStatus.class.getName(), readStatus, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().save(readStatus);
	}

	public void update(ReadStatus readStatus) {
		DBModelUtil.processSecure(ReadStatus.class.getName(), readStatus, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().update(readStatus);
	}

	public void delete(ReadStatus readStatus) {
		DBModelUtil.processSecure(ReadStatus.class.getName(), readStatus, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().delete(readStatus);
	}
	...
	public ReadStatus get(int id) {
		StringBuffer stringBuffer = new StringBuffer();

		stringBuffer.append("FROM ReadStatus ");
		...
		if (list.size() > 0) {
			ReadStatus readStatus = (ReadStatus)list.get(0);

			DBModelUtil.processSecure(ReadStatus.class.getName(), readStatus, DBModelUtil.C_SECURE_TYPE_DECRYPT);
	...
	public ReadStatus getDetail(int id) {
		...
		List<ReadStatus> resultList = this.search(filterParamObject);
	...
	public ReadStatus getDetail(String where) {
        
        List<ReadStatus> resultList = this.search(null, where);
	...
	private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere) {
		...
		if(isCountSQL) {
			...
			sql.appendSQL("SELECT id, owner_id, estimate_id, write_time ");
		}
	...
	public List<ReadStatus> search(JSONObject filterParamObject) {
		return search(filterParamObject, "");
	}

	public List<ReadStatus> search(JSONObject filterParamObject, String extraWhere) {
		...
		List<ReadStatus> resultList = new ArrayList<ReadStatus>();
