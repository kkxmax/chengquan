 ...
 */
public class OpinionDAO extends BaseDataAccessObject {

	private static String VIEW = "opinions_v";

	//处理
	public static final int OPINION_STATUS_NEW = 0;
	public static final int OPINION_STATUS_PROCESSED = 1;


	public void insert(Opinion member) {
		DBModelUtil.processSecure(Opinion.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().save(member);
	}

	public void update(Opinion member) {
		DBModelUtil.processSecure(Opinion.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().update(member);
	}

	public void delete(Opinion member) {
		DBModelUtil.processSecure(Opinion.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
	...
	public Opinion get(int id) {
		...
		stringBuffer.append("FROM Opinion ");
		...
		if (list.size() > 0) {
			Opinion member = (Opinion)list.get(0);

			DBModelUtil.processSecure(Opinion.class.getName(), member, DBModelUtil.C_SECURE_TYPE_DECRYPT);
	...
	public Opinion getDetail(int id) {
		...
		List<Opinion> resultList = this.search(filterParamObject);
	...
	private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere) {
		...
		if(isCountSQL) {
			...
			sql.appendSQL("SELECT id , account_id , content , status , write_time , mobile , status_name ");
		}
	...
	public List<Opinion> search(JSONObject filterParamObject) {
		return search(filterParamObject, "");
	}

	public List<Opinion> search(JSONObject filterParamObject, String extraWhere) {
		...
		if (queryList != null) {
			...
			for(int i = 0; i < listSize; i++) {
				...
				row.setMobile(CommonUtil.toStringDefault(objectArray[5]));
				row.setStatusName(CommonUtil.toStringDefault(objectArray[6]));


				DBModelUtil.processSecure(Opinion.class.getName(), row, DBModelUtil.C_SECURE_TYPE_DECRYPT);
