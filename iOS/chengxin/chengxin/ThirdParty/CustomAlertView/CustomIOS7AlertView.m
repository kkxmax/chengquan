...
const static CGFloat kCustomIOS7AlertViewCornerRadius              = 3;
...
//    if(self.timeout>0)
//    {
//        [self performSelector:@selector(autoHide) withObject:nil afterDelay:self.timeout];
//    }
...
//    UIView *headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, dialogSize.width, 51)];
//    [headerView setBackgroundColor:[UIColor colorWithRed:44.0/255.0 green:178.0/255.0 blue:146.0/255.0 alpha:1.0f]];
//    [dialogContainer addSubview:headerView];
    
    [dialogContainer addSubview:containerView];
    [self addButtonsToView:containerView];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    CGFloat totalWidth = container.bounds.size.width - (([buttonTitles count] + 1) * buttonPadding);
    
    ZDebug(@"totalWidth1: %f", totalWidth);
    
    CGFloat buttonWidth = totalWidth / [buttonTitles count] - 20;
    
    ZDebug(@"buttonWidth2: %f", buttonWidth);
    
    //UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, container.bounds.size.height - buttonHeight + 2, container.bounds.size.width, buttonHeight)];
    
    //[bgView setBackgroundColor:[UIColor whiteColor]];
    //[container addSubview:bgView];

    //int positionY = 5 + container.bounds.size.height - buttonHeight + 2;
    int positionY = container.bounds.size.height - (buttonHeight + 10) ;
    ...
    for (int i = 0; i < [buttonTitles count]; i++) {
        ...
        [closeButton setFrame:CGRectMake(positionX, positionY, buttonWidth, buttonHeight - 12)];

        positionX = positionX + buttonPadding + buttonWidth;
        ...
        [closeButton setTitleColor:WHITE_COLOR forState:UIControlStateHighlighted];
        [closeButton setTitleColor:WHITE_COLOR forState:UIControlStateNormal];
        
        [closeButton.titleLabel setFont:FONT_18];
        [closeButton setBackgroundColor:CYNA_COLOR];
