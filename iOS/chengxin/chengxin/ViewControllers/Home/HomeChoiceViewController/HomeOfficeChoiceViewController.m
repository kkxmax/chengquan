//
//  HomeOfficeChoiceViewController.m
//  chengxin
//
...
@interface HomeOfficeChoiceViewController ()
{
    NSString *choiceKind;
    NSString *choiceCity;
}
...
@implementation HomeOfficeChoiceViewController
...
- (void)viewDidLoad {
    ...
    NSString *strCity = [CommonData sharedInstance].choiceEnterpriseCity;
    ...
    NSString *strEnterkind = [CommonData sharedInstance].choiceEnterpriseKind;
    
    if (strEnterkind.length == 0)
        [self setCategoryButtonSelected:0];
    else
        [self setCategoryButtonSelected:[strEnterkind integerValue]];
...
- (IBAction)choiceCityAction:(id)sender {
    ...
    if(cityButton.tag == (choiceCityButtons.count - 1)) {
        ...
        homeChoiceAllCityVC.homeIndex = SUB_HOME_ENTERPRISE;
...
- (IBAction)businessAction:(id)sender {
    ...
    homeChoiceBusinessVC.mChoice = CHOICE_HOME_ENTERPRISE;
...
- (IBAction)onChoiceAction:(id)sender {
    [CommonData sharedInstance].choiceEnterpriseKind = choiceKind;
    [CommonData sharedInstance].choiceEnterpriseCity = choiceCity;
...
- (IBAction)onResetAction:(id)sender {
    [self setCityButtonSelected:0];
    [self setCategoryButtonSelected:0];
    [CommonData sharedInstance].choiceEnterpriseKind = @"";
    [CommonData sharedInstance].choiceEnterpriseCity = @"";
    [CommonData sharedInstance].choiceEnterpriseIds = @"";
